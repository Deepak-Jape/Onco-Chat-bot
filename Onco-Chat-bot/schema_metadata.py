...
TABLES = {
    ...
    "map_view_population": {
        "purpose": ("Reference geo/population data: one row per city, giving that city's "
                    ...
                    "'population of Sydney', 'cancer case density by city'. Many smaller "
                    "cities have NULL case_ratio/city_population/city_area_km2 -- when "
                    "ranking top/bottom cities by any of these, add NULLS LAST (DESC) or "
                    "filter out NULLs, otherwise rows with no data will dominate the ranking."),
        "pk": "id",
        "columns": {
            "id": "primary key",
