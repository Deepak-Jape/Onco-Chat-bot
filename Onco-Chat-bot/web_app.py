...
def _trial_chips(trial_ids):
    """Render trial IDs as clickable chips -- clicking asks for that trial's executive
    summary (source traceability: user can drill from an aggregate count to the actual
    trials). Uses the SAME data-q + delegated-click flow as the cohort table's
    executive-summary row click (see render_cohort_list / the '.cohort-row' handler),
    rather than a separate inline onclick, so both entry points hit the one proven path."""
    chips = []
    for tid in trial_ids:
        if not tid:
            continue
        follow = f"Give me the executive summary of trial {tid}"
        chips.append(f'<a class="trial-chip" data-q="{esc(follow)}">{esc(tid)}</a>')
    return " ".join(chips)
...
<div class="app">
  <aside class="sidebar">
    <div class="sidebar-head">
      ...
      <a href="__BASE_PATH__/cases" style="display:block;text-align:center;margin-top:8px;font-size:12px;color:#2563eb;text-decoration:none;">Cases Explorer &rarr;</a>
    </div>
...
document.getElementById('threadInner').addEventListener('click', function (e) {
  ...
  if (chip) { if (!busy) ask(chip.getAttribute('data-q')); return; }
});
...
# ===== Cases-per-filter page: dropdown filters over case_filters (from
# merged_clean_global.csv) + a sites table over map_view_population (from
# new_replace_map_view.csv) for the selected country. See tools/get_case_filters.py.
CASES_PAGE = """<!doctype html>
<html><head><meta charset="utf-8"><title>OncoSuite Cases Explorer</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
 * { box-sizing: border-box; }
 body { font-family: -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif; margin:0;
     background:#ffffff; color:#1f2937; }
 .top { padding:16px 24px; border-bottom:1px solid #eef1f6; display:flex; align-items:baseline;
     gap:10px; justify-content:space-between; }
 .top h1 { font-size:16px; margin:0; color:#111827; }
 .top a { color:#2563eb; font-size:13px; text-decoration:none; }
 ...
 .field select { width:100%; padding:8px 10px; border:1px solid #d7deea; border-radius:8px;
     background:#fff; color:#1f2937; font-size:13px; outline:none; }
 .field select:focus { border-color:#2563eb; box-shadow:0 0 0 3px rgba(37,99,235,.10); }

 .summary { display:flex; gap:16px; margin-bottom:22px; flex-wrap:wrap; }
 .stat { flex:1; min-width:180px; border:1px solid #e5e9f0; border-radius:12px; padding:14px 16px;
     background:#f7f9fc; }
 .stat .label { font-size:12px; color:#6b7280; margin-bottom:4px; }
 .stat .value { font-size:24px; font-weight:700; color:#111827; }
 .stat .sub { font-size:12px; color:#9aa4b2; margin-top:2px; }

 h2.section { font-size:14px; color:#111827; margin:26px 0 10px; }
 table.sites { width:100%; border-collapse:collapse; font-size:13px; }
 table.sites th { text-align:left; padding:8px 10px; border-bottom:2px solid #e5e9f0;
     color:#374151; font-weight:600; background:#f7f9fc; }
 table.sites td { padding:8px 10px; border-bottom:1px solid #f0f2f6; color:#1f2937; }
 table.sites tr:hover td { background:#f7f9fc; }
 .empty { color:#9aa4b2; font-size:13px; padding:20px 0; }
 .err { color:#b42318; font-size:13px; padding:10px 0; }
</style>
</head><body>
<div class="top">
  <h1>Cases Explorer</h1>
  <a href="__BASE_PATH__/">&larr; Back to chat</a>
</div>
<div class="wrap">
...
const BASE_PATH = "__BASE_PATH__";
const COLS = [
  ["country", "Country"], ["organ", "Organ"], ["histology", "Histology"],
  ["biomarkers", "Biomarkers"], ["cancer_stage", "Cancer Stage"],
  ["line_of_therapy", "Line of Therapy"],
];
// Filters chosen so far. Empty string = "not picked yet" (server treats that as "All").
let selected = {country: "", organ: "Lung", histology: "", biomarkers: "",
                cancer_stage: "", line_of_therapy: ""};
...
function renderFilters(options) {
  ...
  box.innerHTML = COLS.map(([key, label]) => {
    const vals = options[key] || [];
    const opts = ['<option value="">Any</option>']
      .concat(vals.map(v => `<option value="${esc(v)}" ${selected[key] === v ? 'selected' : ''}>${esc(v)}</option>`));
    return `<div class="field"><label>${label}</label>
      <select data-key="${key}" onchange="onFilterChange(this)">${opts.join('')}</select></div>`;
  }).join('');
}
...
function renderSummary(data) {
  ...
  if (!c) {
    box.innerHTML = '<div class="err">No data for this exact combination — pick a value already offered by another dropdown.</div>';
    return;
  }
  box.innerHTML = `
    <div class="stat"><div class="label">Annual new cases</div>
      <div class="value">${fmt(c.annual_new_cases)}</div>
      <div class="sub">${esc(c.country)} &middot; ${esc(c.organ)}</div></div>
    <div class="stat"><div class="label">Country population</div>
      <div class="value">${fmt(c.population)}</div></div>
    <div class="stat"><div class="label">Matching sites</div>
      <div class="value">${fmt(data.sites_count)}</div></div>`;
}

function renderSites(sites) {
  const box = document.getElementById('sitesBox');
  if (!sites.length) { box.innerHTML = '<div class="empty">No sites found for this country.</div>'; return; }
  const rows = sites.map(s => `<tr>
      <td>${esc(s.city || s.admin_name || '—')}</td>
      <td>${esc(s.admin_name || '—')}</td>
      <td>${esc(s.country)}</td>
      <td>${fmt(s.city_population)}</td>
      <td>${fmt(s.case_ratio)}</td>
    </tr>`).join('');
  box.innerHTML = `<table class="sites"><thead><tr>
      <th>City</th><th>Region</th><th>Country</th><th>Population</th><th>Case ratio</th>
    </tr></thead><tbody>${rows}</tbody></table>`;
}

async function refresh() {
  try {
    const options = await getJSON('/api/cases/options', selected);
    // A prior selection may no longer be valid for the new facet set -- drop it
    // rather than silently querying a combination the data doesn't have.
    for (const [key] of COLS) {
      if (selected[key] && !(options[key] || []).includes(selected[key])) selected[key] = '';
    }
    renderFilters(options);
    const data = await getJSON('/api/cases', selected);
    renderSummary(data);
    renderSites(data.sites);
  } catch (e) {
    document.getElementById('summary').innerHTML = `<div class="err">${esc(e.message)}</div>`;
  }
}
...
CASES_PAGE_HTML = CASES_PAGE.replace("__BASE_PATH__", BASE_PATH)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _route_get(self):
        ...
        if path in ("/", "/index.html"):
            ...
            self._send(CASES_PAGE_HTML)
        elif path in ("/api/health", "/health"):
            self._send(json.dumps({"status": "ok", "service": "oncosuite"}),
                       "application/json")
        elif path == "/api/cases/options":
            self._handle_case_options()
        elif path == "/api/cases":
            self._handle_case_lookup()
        else:
            self.send_error(404)

    def _query_params(self):
        """Single-valued query params from the request URL, e.g. ?country=India."""
        raw = parse_qs(urlparse(self.path).query)
        return {k: v[0] for k, v in raw.items() if v}

    def _handle_case_options(self):
        """GET /api/cases/options?country=&organ=&... -> faceted dropdown options
        for the /cases page (tools/get_case_filters.get_filter_options)."""
        try:
            from tools.get_case_filters import FILTER_COLUMNS, get_filter_options
            params = self._query_params()
            selected = {k: params.get(k) for k in FILTER_COLUMNS}
            out = get_filter_options(selected)
        except Exception as e:
            self._send(json.dumps({"error": str(e)}), "application/json", 500)
            return
        self._send(json.dumps(out, default=str), "application/json")

    def _handle_case_lookup(self):
        """GET /api/cases?country=&organ=&... -> case count + matching sites
        (tools/get_case_filters.run)."""
        try:
            from tools.get_case_filters import FILTER_COLUMNS, run as get_cases
            params = self._query_params()
            selected = {k: params.get(k) for k in FILTER_COLUMNS}
            out = get_cases(selected)
        except Exception as e:
            self._send(json.dumps({"error": str(e)}), "application/json", 500)
            return
        self._send(json.dumps(out, default=str), "application/json")
    ...
    def _handle_map_json(self):
        """Programmatic JSON API: POST /api/map {"q": "..."} -> filtered map points.

        Unlike /api/ask, this NEVER returns prose -- it turns the question into a
        filtered SQL query against map_view_population (tools/get_map_data.py) and
        returns the matching rows as plain {lat, lng, city, country, caseRatio, ...}
        points, for a map to plot."""
        try:
            length = int(self.headers.get("Content-Length") or 0)
            payload = json.loads(self.rfile.read(length) or b"{}")
        except Exception as e:
        ...
        if not q:
            ...
            return
        try:
            from tools.get_map_data import run as get_map_data
            out = get_map_data(q)
            out["question"] = q
        except Exception as e:
            self._send(json.dumps({"question": q, "error": str(e)}), "application/json", 500)
