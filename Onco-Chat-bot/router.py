...
def _check_general_knowledge_confidence(user_message, answer):
    """Self-critique pass for the general-knowledge fallback.

    There's no data here (no rows, no snippets) to run text_to_sql's verify_answer
    grounding check against, so this checks something different: overconfident
    phrasing -- specific numbers/IDs/dates/names stated as flat fact when the model
    has no source to have looked them up from. Fails open (ok=True) on any LLM error,
    same rationale as verify_answer -- a missed catch is better than blocking a real
    answer."""
    import llm_client
    checker = (
        "You are reviewing an answer generated from general medical/oncology knowledge, "
        "NOT from a verified trial database. Check ONLY for overconfident phrasing: "
        "specific numbers, trial IDs, dates, or names stated as flat fact without any "
        "hedge (e.g. 'approximately', 'typically', 'in most studies'), where that detail "
        "is the kind that varies by source and the model can't have looked up. "
        "Well-established general medical facts stated plainly are FINE -- only flag "
        "invented-sounding specifics.\n\n"
        "Respond in EXACTLY this format:\n"
        "VERDICT: OK\n"
        "or\n"
        "VERDICT: OVERCONFIDENT\nPROBLEMS: <one line per overconfident claim>\n"
    )
    messages = [
    ...
    try:
        ...
        return True, "checker unavailable (failed open)"
    if re.search(r"VERDICT:\s*OVERCONFIDENT", verdict, re.IGNORECASE):
        problems = ""
        m = re.search(r"PROBLEMS:\s*(.+)", verdict, re.IGNORECASE | re.DOTALL)
        if m:
            problems = m.group(1).strip()
        return False, problems or "overconfident phrasing detected"
    return True, "ok"


def _soften_general_knowledge(user_message, answer, problems):
    """Rewrite once with the flagged claims hedged or dropped. Best-effort -- caller
    keeps the original answer if this fails."""
    import llm_client
    system = (
        "You previously answered a clinical/oncology question from general knowledge. "
        "A reviewer flagged some claims as stated too confidently for specifics that can "
        "vary by source. Rewrite the answer, adding appropriate hedging (e.g. "
        "'approximately', 'typically', 'this can vary') to the flagged claims, or removing "
        "them if you cannot hedge them honestly. Keep the rest of the answer as-is, "
        "including the opening note that this is general knowledge, not from the trial "
        "dataset.\n\n"
        f"FLAGGED PROBLEMS:\n{problems}\n\nPREVIOUS ANSWER:\n{answer}"
    )
...
def _general_knowledge_response(user_message):
    ...
    # Self-critique pass (see _check_general_knowledge_confidence docstring for why
    # this isn't the same as verify_answer's grounding check).
    ok, problems = _check_general_knowledge_confidence(user_message, text)
    if not ok:
        try:
            softened = _soften_general_knowledge(user_message, text, problems)
            if softened and softened.strip():
                text = softened
        except Exception:
            pass  # best-effort -- keep the original answer if softening fails
