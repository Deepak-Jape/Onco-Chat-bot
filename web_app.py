...
def render_answer(resp, question=""):
    ...
    # A rendered answer already says everything the reader needs, so dumping the
    # raw tool_result underneath it just shows them the plumbing -- that is how a
    # full JSON blob ended up below the trial-comparison table. The dump is a
    # DEBUG aid for response shapes that have no renderer yet, so only fall back
    # to it when there is no synthesis to show.
    if synthesis_html:
        return banner + synthesis_html

    # fallback: pretty JSON (no renderer for this shape yet)
...
# CORS allowlist -- unset by default. On the VM, Caddy serves ctsearch and
# proxies /chat-bot* to this app on the SAME origin (204.168.157.213.sslip.io),
# so no CORS is needed there. This exists for local dev only: running ctsearch's
# Vite dev server against a remote/differently-hosted API, where the browser
# origins genuinely differ. Comma-separated in the env; "*" allows any origin.
_CORS_ORIGINS = [
    o.strip() for o in os.environ.get("ONCOSUITE_CORS_ORIGINS", "").split(",") if o.strip()
]
...
class Handler(BaseHTTPRequestHandler):
    def _cors_headers(self):
        ...
        allow = "*" if "*" in _CORS_ORIGINS else (origin if origin in _CORS_ORIGINS else None)
        if not allow:
            return
    ...
    def _handle_ask_fast(self):
        ...
        # "what about phase 2?" continues the previous question rather than
        # starting a new one. Resolving it HERE -- before routing, chart
        # selection or filter extraction -- means every one of those sees the
        # full question and needs no follow-up handling of its own.
        try:
            # Typos first: "top bimoarkers" must be repaired BEFORE routing or
            # follow-up resolution, both of which match on exact words.
            from answer_fast import correct_typos, resolve_followup
            q = correct_typos(q)
            q = resolve_followup(q, session_id)
        except Exception:
            pass  # an unresolvable follow-up is still answerable as written
        ...
        if _CORS_ORIGINS:
            ...
            allow = "*" if "*" in _CORS_ORIGINS else (origin if origin in _CORS_ORIGINS else None)
            if allow:
                self.wfile.write(f"Access-Control-Allow-Origin: {allow}\r\n".encode("utf-8"))
                if allow != "*":
                    self.wfile.write(b"Vary: Origin\r\n")
        self.wfile.write(b"\r\n")
        self.wfile.flush()
            ...
            def worker():
                try:
                    ...
                    if detect_analytics(q):
                        ...
                        # These never reach the router, which is what normally
                        # records the turn -- so record it here, otherwise the
                        # NEXT question's follow-up resolution has no subject to
                        # carry forward.
                        try:
                            from conversation import conversations
                            conversations.add_user(session_id, q)
                        except Exception:
                            pass
                        return
    ...
    def _handle_ask_stream(self):
        ...
        if _CORS_ORIGINS:
            ...
            allow = "*" if "*" in _CORS_ORIGINS else (origin if origin in _CORS_ORIGINS else None)
            if allow:
                cors = f"Access-Control-Allow-Origin: {allow}\r\n"
                if allow != "*":
                    cors += "Vary: Origin\r\n"
        head = (
            "HTTP/1.1 200 OK\r\n"
