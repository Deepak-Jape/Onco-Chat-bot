...
# # # #         return ""
# # # #     q = (question or "").lower()
# # # #     # If the user specifically asks about eligibility, surface the criteria at the TOP.
# # # #     # Show only the side they asked for (inclusion / exclusion), or both if unspecified.
# # # #     if any(w in q for w in ("eligib", "criteria", "inclusion", "exclusion", "who can join",
# # # #                             "who is eligible", "enrollment criteria")):
# # # #         cohorts = tr.get("cohorts") or []
# # # #         crit = cohorts[0] if cohorts else {}
# # # #         want_incl = "inclusion" in q or not ("exclusion" in q)
# # # #         want_excl = "exclusion" in q or not ("inclusion" in q)
# # # #         blocks = []
# # # #         if want_incl:
# # # #             blocks.append(render_criteria_block("Inclusion criteria",
# # # #                                                 crit.get("eligibility_inclusion_criteria")))
# # # #         if want_excl:
# # # #             blocks.append(render_criteria_block("Exclusion criteria",
# # # #                                                 crit.get("eligibility_exclusion_criteria")))
# # # #         blocks = [b for b in blocks if b]
# # # #         if blocks:
# # # #             return (f'<div class="answer" style="border-left-color:#38b48b">'
# # # #                     f'<span class="answer-lbl">Eligibility for {esc(tr.get("nct_id"))}</span>'
# # # #                     + "".join(blocks) + '</div>')
# # # #         return (f'<div class="answer"><span class="answer-lbl">Eligibility</span>'
# # # #                 f'<span class="answer-val">Not recorded for this trial</span></div>')
...
# # # #             for a in aes
# # # #         )
# # # #         out.append('<h4>Adverse events</h4>'
...
# # # #                 v = json.dumps(to_plain(v), indent=2)
# # # #                 rows.append(f'<div class="crit-row"><span class="crit-key">{esc(k)}</span>'
# # # #                             f'<span class="crit-val"><pre>{esc(v)}</pre></span></div>')
# # # #             else:
# # # #                 rows.append(f'<div class="crit-row"><span class="crit-key">{esc(k)}</span>'
# # # #                             f'<span class="crit-val">{esc(v)}</span></div>')
# # # #         body = "".join(rows)
# # # #     else:
# # # #         body = f'<pre>{esc(json.dumps(to_plain(summary), indent=2))}</pre>'
# # # #     return f'<div class="card"><h3>Summary</h3>{body}</div>'
...
# # # #     parts.append(render_summary(tr.get("summary")))
...
# # # #     if tr.get("error"):
# # # #         return f'<div class="card error">{esc(tr["error"])}</div>'
# # # #     rows = tr.get("rows") or tr.get("results") or []
# # # #     if not rows:
# # # #         return f'<div class="card"><pre>{esc(json.dumps(to_plain(tr), indent=2))}</pre></div>'
# # # #     # generic table over whatever keys the rows have
# # # #     keys = list(rows[0].keys())
# # # #     thead = "".join(f"<th>{esc(k)}</th>" for k in keys)
# # # #     body = "".join(
# # # #         "<tr>" + "".join(f"<td>{esc(row.get(k))}</td>" for k in keys) + "</tr>"
# # # #         for row in rows
# # # #     )
# # # #     return f'<div class="card"><table><thead><tr>{thead}</tr></thead><tbody>{body}</tbody></table></div>'


# # # # def render_clarification(resp):
# # # #     terms = resp.get("unmatched_terms", [])
# # # #     lis = "".join(f"<li>{esc(t.get('term'))} <span class='muted'>(field: {esc(t.get('group'))})</span></li>"
# # # #                   for t in terms)
# # # #     return ('<div class="card warn"><h3>Need clarification</h3>'
# # # #             "<p>I couldn't match these terms to known values &mdash; please rephrase or pick a valid value:</p>"
# # # #             f"<ul>{lis}</ul></div>")
...
# # # #         return banner + render_trial_detail(tr, question)
# # # #     if tool == "search_trials":
# # # #         return banner + render_search_results(tr)
# # # #     if tool == "get_competitive_landscape":
# # # #         return banner + render_landscape(tr)
# # # #     # fallback: pretty JSON
# # # #     return banner + f'<div class="card"><pre>{esc(json.dumps(to_plain(tr), indent=2))}</pre></div>'
...
# # # #  body { font-family: -apple-system, Segoe UI, Roboto, sans-serif; margin:0; background:#0f1420; color:#e6eaf2; }
# # # #  .wrap { max-width: 860px; margin: 0 auto; padding: 28px 20px 60px; }
# # # #  h1 { font-size: 20px; margin: 0 0 4px; }
# # # #  .sub { color:#8b95a7; font-size:13px; margin:0 0 20px; }
# # # #  form { display:flex; gap:10px; margin-bottom: 22px; }
# # # #  input[type=text] { flex:1; padding:12px 14px; border-radius:10px; border:1px solid #2a3446;
# # # #      background:#161c2b; color:#e6eaf2; font-size:15px; }
# # # #  button { padding:12px 20px; border:0; border-radius:10px; background:#4c8bf5; color:#fff;
# # # #      font-size:15px; font-weight:600; cursor:pointer; }
# # # #  .meta { font-size:12px; color:#8b95a7; margin-bottom:10px; }
# # # #  .answer { background:#13233a; border:1px solid #2f5fa8; border-left:4px solid #4c8bf5;
...
# # # #  .card.error { border-color:#b3392f; color:#ffb3ad; }
# # # #  .card.warn { border-color:#b3892f; }
# # # #  h2 { font-size:17px; margin:2px 0 6px; }
# # # #  h3 { font-size:15px; margin:2px 0 8px; }
...
# # # #  .tag { display:inline-block; font-size:11px; background:#22304a; color:#9fc0ff; padding:3px 8px;
...
# # # #  .crit-key { min-width:150px; color:#9fb3d9; }
# # # #  .crit-val { flex:1; }
# # # #  .arm { border-left:3px solid #4c8bf5; padding:8px 12px; margin:10px 0; background:#131a28; border-radius:0 8px 8px 0; }
# # # #  .arm-desc { font-size:13px; color:#c7cfdd; margin:4px 0; }
# # # #  .arm-drug { font-size:12px; color:#9fb3d9; }
# # # #  .pill { font-size:11px; background:#2a3446; padding:2px 7px; border-radius:5px; color:#c7cfdd; }
# # # #  .count { color:#8b95a7; font-size:13px; }
# # # #  .muted { color:#8b95a7; }
# # # #  table { width:100%; border-collapse:collapse; font-size:13px; }
# # # #  th,td { text-align:left; padding:6px 8px; border-bottom:1px solid #232c3d; }
# # # #  th { color:#9fb3d9; }
# # # #  pre { white-space:pre-wrap; font-size:12px; color:#c7cfdd; margin:0; }
# # # #  a { color:#6fa8ff; }
# # # #  .examples { font-size:12px; color:#8b95a7; margin-top:6px; }
# # # #  .examples a { margin-right:12px; }
# # # # </style></head>
# # # # <body><div class="wrap">
# # # #  <h1>OncoSuite Clinical Trial Assistant</h1>
# # # #  <p class="sub">Ask in plain English. Answers come from the oncosuite_gold database.</p>
# # # #  <form method="get" action="/">
# # # #    <input type="text" name="q" value="{q}" placeholder="e.g. what is eligibility criteria for trial NCT03706690" autofocus>
# # # #    <button type="submit">Ask</button>
# # # #  </form>
# # # #  <div class="examples">Try:
# # # #    <a href="/?q=Show+me+recruiting+Phase+3+trials+for+NSCLC+KRAS">recruiting Phase 3 NSCLC KRAS</a>
# # # #    <a href="/?q=Tell+me+about+NCT06881784">trial NCT06881784</a>
# # # #    <a href="/?q=what+is+eligibility+criteria+for+trial+NCT03706690">eligibility NCT03706690</a>
# # # #  </div>
...
# # # #                 answer_html = render_answer(resp, q)
# # # #             except Exception as e:  # keep the page alive on any tool error
# # # #                 answer_html = f'<div class="card error">Error: {esc(e)}</div>'
# # # #         body = PAGE.format(q=html.escape(q), answer=answer_html).encode("utf-8")
...
# # #     # General "tell me about X" / "what is this trial" / "summary" / "purpose" --
# # #     # give an actual written sentence up top, not just the raw data card below.
# # #     # This is deliberately checked AFTER eligibility (more specific) and BEFORE
# # #     # the narrower DIRECT_FIELDS loop (single-attribute questions), so a broad
# # #     # "tell me about" doesn't get outcompeted by e.g. a bare "phase" substring
# # #     # match, but a specific "what phase is X in" still gets its own precise answer.
# # #     if any(w in q for w in ("tell me about", "what is this trial", "overview", "summary",
# # #                             "describe", "purpose", "what is nct", "about nct")):
# # #         nct = esc(tr.get("nct_id"))
# # #         phase = tr.get("trial_phase") or "an unspecified phase"
# # #         status = (tr.get("study_status") or "an unknown status").lower()
# # #         sponsor = tr.get("sponsor_name") or "an unspecified sponsor"
# # #         title = tr.get("official_title") or ""
# # #         cohorts = tr.get("cohorts") or []
# # #         cond_bits = []
# # #         if cohorts:
# # #             c = cohorts[0]
...
# # #         sponsor_clean = (sponsor or "").rstrip(".")
# # #         sentence = f"{nct} is {esc(phase)}, currently {esc(status)}, sponsored by {esc(sponsor_clean)}."
# # #         if title:
# # #             t = title.strip().rstrip(".")
# # #             sentence += f" It studies: {esc(t)}."
# # #         if cond_bits:
# # #             sentence += f" Cohort focus: {esc(' with '.join(cond_bits))}."
# # #         return (f'<div class="answer" style="border-left-color:#38b48b">'
# # #                 f'<span class="answer-lbl">About {nct}</span>'
# # #                 f'<p class="answer-val" style="font-size:15px;font-weight:400;line-height:1.5;margin-top:6px">{sentence}</p>'
# # #                 f'</div>')
...
# #     if filt_bits:
# #         blocks.append(f'<p class="count">Filters &mdash; {esc("; ".join(filt_bits))}</p>')
...
# #               ", ".join((r.get("example_trial_ids") or [])[:3])]
...
#     """Minimal, dependency-free markdown -> HTML for the specific subset synthesis.py
#     generates: **bold headers**, pipe tables, bullet lines, _italic notes_. Not a
#     general markdown parser -- deliberately narrow, matched to what we actually emit."""
#     lines = md.split("\n")
#     out = []
#     table_buf = []
...
#                    "".join(f"<th>{esc(c)}</th>" for c in head_cells) + "</tr></thead><tbody>")
#         for r in rows[1:]:
...
#             out.append(f"<h4 class='synth-h'>{esc(s.strip('*'))}</h4>")
#         elif s.startswith("- "):
#             out.append(f"<li>{esc(s[2:])}</li>")
#         elif s.startswith("_") and s.endswith("_") and len(s) > 1:
#             out.append(f"<p class='muted'>{esc(s.strip('_'))}</p>")
#         elif s:
#             out.append(f"<p>{esc(s)}</p>")
#     flush_table()
#     html = "\n".join(out)
#     # wrap consecutive <li> runs in <ul>
#     html = html.replace("<li>", "<ul><li>", 1) if "<li>" in html else html
#     return html
...
#         return ""
#     mode_label = "LLM-generated" if synthesis.get("mode") == "llm" else "computed directly from verified data, no LLM"
#     return (
#         '<div class="card" style="border-color:#4c8bf5;border-width:2px">'
#         f'<div class="tag" style="background:#1a2a4a">Analysis \u2014 {esc(mode_label)}</div>'
...
#     if tool == "get_trial_detail":
#         return banner + synthesis_html + render_trial_detail(tr, question)
#     if tool == "search_trials":
...
#  .synth-body p {{ margin: 4px 0; line-height: 1.5; }}
#  .synth-body ul {{ margin: 4px 0; padding-left: 20px; }}
# </style></head>
# <body><div class="wrap">
#  <h1>OncoSuite Clinical Trial Assistant</h1>
#  <p class="sub">Ask in plain English. Answers come from the oncosuite_gold database.</p>
...
def render_synthesis(synthesis: dict) -> str:
    ...
    if mode == "general_knowledge":
        # NOT from the database -- flag it loudly (amber) so AI knowledge is never
        # mistaken for verified trial data.
        return (
            '<div class="card" style="border-color:#d9822b;border-width:2px;background:#fffaf0">'
            '<div class="tag" style="background:#fbe7c6;color:#8a5a12">'
            '&#9888; AI-generated &mdash; general knowledge, NOT from the trial database</div>'
            f'<div class="synth-body">{render_markdown_lite(synthesis["text"])}</div>'
            '<div class="muted" style="margin-top:8px;font-size:11px">'
            'This answer was generated by the AI model from its own knowledge because the '
            'requested information is not in the oncosuite_gold database. Verify before '
            'relying on it.</div>'
            '</div>'
        )
...
def render_answer(resp, question=""):
    ...
    if mode == "clarification_needed":
        # DB doesn't have these exact terms. Show the honest "not in dataset" note, and
        # ALSO offer an AI general-knowledge answer below it (clearly flagged).
        clar = render_clarification(resp)
        gk = ""
        try:
            from router import _general_knowledge_response
            g = _general_knowledge_response(question)
            if g:
                gk = render_synthesis(g.get("synthesis"))
        except Exception:
            pass
        return banner + clar + gk
    if mode == "out_of_scope_policy_needed":
        return banner + ('<div class="card warn"><h3>Out of scope</h3>'
                         f'<p>{esc(resp.get("note"))}</p></div>')
    if mode == "cohort_list":
    ...
    if tool == "text_to_sql":
        ...
        if rows:
            ...
            table_html = f'<div class="card">{simple_table(headers, body, title="Query results")}</div>'
        sql = (tr or {}).get("sql")
        sql_html = (f'<details class="card"><summary class="muted" style="cursor:pointer">'
                    f'Show SQL used</summary><pre>{esc(sql)}</pre></details>') if sql else ""
        if not synthesis_html and not table_html:
            return banner + '<div class="card">I found no matching data for that question.</div>'
        # chart first (visual), then the answer text, then the detail table + SQL
        return banner + chart_html + synthesis_html + table_html + sql_html
