...
// Mean centre of supplied points, so the viewport frames the real data instead
// of the hardcoded Houston focus used by the placeholder.
function centreOf(points) {
  if (!points || !points.length) return { lng: -95.3698, lat: 29.7604, zoom: 6 };
  let lng = 0;
  let lat = 0;
...
/* `points` is optional: pass [{latitude, longitude, value, name}] to plot real
   site coordinates. With no points the component keeps its original
   randomAround() placeholder behaviour, so existing ctsearch usage is
   unchanged. */
export default function UsHeatMap({ points = null }) {
  const mapRef = useRef(null);
  const initial = useMemo(() => centreOf(points), [points]);
