...
// min/max.
// Exported alongside TrialDurationChart so other apps can feed the same raw
// API points and get identical rows (ISO codes, flags, risk band, tooltip
// percentiles) rather than reimplementing the mapping.
export const buildTrialDurationRows = (points = [], minTrials = 0, maxTrials = 1) => {
...
// Exported so other apps (OncoSuite) can render the real chart instead of a
// look-alike. Props unchanged: rows = [{code, iso, risk, total, segments}].
export function TrialDurationChart({ rows, axisMax }) {
