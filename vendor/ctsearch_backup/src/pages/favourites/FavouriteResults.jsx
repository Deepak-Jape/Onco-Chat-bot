    ...
    const metrics = [
        {
            ...
            badgeText: <SafeRender>{`↓ ${result?.operational_metrics?.value?.recruitment_rate[1]} % vs benchmark`}</SafeRender>,
        },
        {
            title: "Dropout Rate",
            value: <SafeRender>{`${result?.operational_metrics?.value?.dropout_rate[0]}%`}</SafeRender>,
            valueUnit: "of Patients Discontinue",
            valueColor: "text-blue-600",
            badgeColor: "text-green-600 bg-green-50",
            badgeText: <SafeRender>{`↑ ${result?.operational_metrics?.value?.dropout_rate[1]} % vs benchmark`}</SafeRender>,
        },
        {
            title: "Accrual",
            value:<SafeRender>{`${result?.operational_metrics?.value?.accrual[0]}%`}</SafeRender>,
            valueUnit: "Actual vs Planned",
            valueColor: "text-green-600",
            badgeColor: "text-red-500 bg-red-50",
            badgeText:<SafeRender>{`↓ ${result?.operational_metrics?.value?.accrual[1]} % vs benchmark`}</SafeRender>,
        },
        {
            title: "TTPE",
            value: <SafeRender>{`${result?.operational_metrics?.value?.ttpe[0]} Days`}</SafeRender>,
            valueUnit: "to Primary Endpoint",
            valueColor: "text-limeGreen",
            badgeColor: "text-red-500 bg-red-50",
            badgeText: <SafeRender>{`↓ ${result?.operational_metrics?.value?.ttpe[1]} % vs benchmark`}</SafeRender>,
        },
    ];
