...
import { DRUG_FILTER_SECTIONS } from "../../utils/helpers/helper";
...
export default function TrialsFilterDrug({
    // searchSelections,
    // setSearchSelections,
    // setSelectedFilters
    // onToggleCheckBox
    ...
    const [activeSection, setActiveSection] = useState(DRUG_FILTER_SECTIONS[0].title);
    ...
    const PLACEHOLDER_KEY_MAP = DRUG_FILTER_SECTIONS.flatMap(
    ...
    const PLACEHOLDER_WORD_KEY_MAP = DRUG_FILTER_SECTIONS.flatMap(
        (section) => section.filters,
    ).reduce((acc, filter) => {
        if (filter.placeholder) {
            filter.placeholder
    ...
    useEffect(() => {
        ...
        const fire = () => {
            // Mark as handled only when we actually dispatch, so a skipped run
            // doesn't cache the key and block the real fetch on the next run.
    ...
    const filterConfigMap = useMemo(() => {
        const map = {};
        DRUG_FILTER_SECTIONS.forEach((section) => {
    ...
    const VALID_FILTER_KEYS = new Set(
        DRUG_FILTER_SECTIONS.flatMap((section) => section.filters.map((f) => f.key)),
    );
                                ...
                                >
                                    ...
                                    >
                                        <Box sx={{ display: "flex", height: "100%" }}>
                                            ...
                                            >
                                                {DRUG_FILTER_SECTIONS.map((section) => (
                                                    <Box
                                                        key={section.title}
