  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={rowValueStyle}>{d.orr ?? "—"}%</span>

        <span style={rowLabelStyle}>{yMetric} (Safety)</span>
        <span style={rowValueStyle}>{d.sae ?? "—"}</span>
...
function EfficacyVsSafety({
  ...
  const [colorBy, setColorBy] = useState(efficacyfilter[0]);
  const [xMetric, setXMetric] = useState("ORR");
  const [yMetric, setYMetric] = useState("SAE");

  const config = COLOR_BY_CONFIG[colorBy] || COLOR_BY_CONFIG[efficacyfilter[0]];
  ...
  const filteredData = useMemo(
    () => filterByLeftAndTop(data, selections, topFilters, scope, true),
    [data, selections, topFilters, scope]
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, fontSize: 12, color: "rgba(0,0,0,0.6)" }}>
        <Dropdown options={efficacyfilter} value={colorBy} onChange={setColorBy} />
        <Dropdown options={["ORR", "PFS", "OS", "DCR"]} value={xMetric} onChange={setXMetric} />
        <Dropdown options={["SAE", "AE", "Grade 3+", "Discontinuation"]} value={yMetric} onChange={setYMetric} />
      </div>
      {!hasData ? (
        <div
          style={{
      ...
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={{ top: 10, right: 20, bottom: 20, left: 0 }}>
          ...
          <XAxis
            ...
            label={{ value: "SAE", position: "insideBottom", offset: -10, fontSize: 12 }}
          />
          <YAxis
            type="number"
            dataKey="orr"
            ...
            domain={[0, 100]}
            tick={{ fontSize: 11, fill: "rgba(0,0,0,0.6)" }}
            label={{ value: "ORR (%)", angle: -90, position: "insideLeft", fontSize: 12 }}
          />
